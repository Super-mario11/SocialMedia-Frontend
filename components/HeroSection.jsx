function HeroSection({ creator, onUnlock }) {
  return (
    <section id="hero" className="relative overflow-hidden rounded-3xl border border-white/10 bg-zinc-900 p-5 md:p-7">
      ...
      <div className="relative md:grid md:grid-cols-[220px,1fr] md:items-center md:gap-8">
        ...
        />
        <div>
          <h1 className="mt-4 text-center text-2xl font-bold tracking-tight md:mt-0 md:text-left md:text-4xl">
            {creator.name}
          </h1>
          <p className="mt-2 text-center text-sm text-zinc-300 md:text-left md:text-base">{creator.tagline}</p>
          <p className="mt-2 text-center text-xs text-zinc-400 md:text-left md:text-sm">{creator.bio}</p>

          <div className="mt-5 grid grid-cols-2 gap-3 md:max-w-sm">
            <div className="rounded-2xl bg-white/5 p-3 text-center">
              <p className="text-lg font-semibold">{creator.postsCount}</p>
              <p className="text-xs uppercase tracking-wide text-zinc-400">Posts</p>
            </div>
            <div className="rounded-2xl bg-white/5 p-3 text-center">
              <p className="text-lg font-semibold">{creator.subscribersCount.toLocaleString()}</p>
              <p className="text-xs uppercase tracking-wide text-zinc-400">Subscribers</p>
            </div>
          </div>
          ...
          <div className="mt-5 space-y-3 md:flex md:max-w-md md:gap-3 md:space-y-0">
            ...
            <button
              type="button"
              onClick={onUnlock}
              className="w-full rounded-xl bg-rose-500 px-4 py-3 text-sm font-semibold text-white transition active:scale-[0.99]"
            >
